...
def ffnn(inputs, num_hidden_layers, hidden_size, output_size, dropout, output_weights_initializer=None):
    ...
    output_weights = tf.compat.v1.get_variable("output_weights", [shape(current_inputs, 1), output_size],
                                     initializer=output_weights_initializer or tf.compat.v1.truncated_normal_initializer(stddev=0.02))
...
def multi_head_attention(query, key, value, num_heads, attention_size, 
                        attention_mask=None, dropout_rate=0.0, scope="multi_head_attention"):
    """
    Enhanced multi-head attention implementation with residual connections and layer normalization.
    ...
    Args:
        ...
        dropout_rate: Dropout rate for attention probabilities
        scope: Variable scope name
        
    Returns:
        Context layer after attention and residual connection
    """
    with tf.compat.v1.variable_scope(scope):
        ...
        # Linear projections
        Q = tf.compat.v1.layers.dense(query, attention_size, activation=tf.nn.relu)  # [N, T_q, C]
        K = tf.compat.v1.layers.dense(key, attention_size, activation=tf.nn.relu)    # [N, T_k, C]
        V = tf.compat.v1.layers.dense(value, attention_size, activation=tf.nn.relu)  # [N, T_k, C]
        ...
        if attention_mask is not None:
            key_masks = tf.cast(attention_mask, tf.float32)
        ...
        # Activation
        attention_probs = tf.nn.softmax(outputs)  # [h*N, T_q, T_k]
        
        # Dropout on attention weights
        attention_probs = tf.nn.dropout(attention_probs, rate=dropout_rate)
        
        # Weighted sum
        outputs = tf.matmul(attention_probs, V_)  # [h*N, T_q, C/h]
...
def get_prf_mentions_for_all_documents(gold_docs, pred_docs):
    """
    Compute mention-level precision, recall and F1 across all documents.
    
    Args:
        gold_docs: List of gold documents (each with 'clusters' field)
        pred_docs: Dictionary mapping doc_key to predicted clusters
    ...
    for gold_doc in gold_docs:
        ...
        pred_clusters = pred_docs[doc_key]
        
        # Flatten all mentions in gold clusters
        gold_mentions = set()
        for cluster in gold_clusters:
            for mention in cluster:
                gold_mentions.add(tuple(mention))
        
        # Flatten all mentions in predicted clusters
        pred_mentions = set()
        for cluster in pred_clusters:
            for mention in cluster:
                pred_mentions.add(tuple(mention))
        
        num_gold += len(gold_mentions)
        num_pred += len(pred_mentions)
        num_correct += len(gold_mentions & pred_mentions)
    
    precision = maybe_divide(num_correct, num_pred)
    recall = maybe_divide(num_correct, num_gold)
    f1 = maybe_divide(2 * precision * recall, precision + recall)
